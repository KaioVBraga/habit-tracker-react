...
const ModalGoalCreator: React.FC<Props> = (props) => {
  ...
  const handleHabits = useCallback(
    async (habits) => {
      ...
      await api.post(`users/${user.id}/habits`, { habits }).then((res) => {
        const newGoals = [...goals];

        const formatedHabits = res.data.map((habit: any) => ({
          ...habit,
          marks: [],
        }));

        newGoals[activeHabit.goalIndex].habits = newGoals[
          activeHabit.goalIndex
        ].habits.concat(formatedHabits);
  ...
  const handleGoal = useCallback(
    async (goal) => {
      ...
      api
        .post(`users/${user.id}/goals`, { ...goal, type: category, active: 1 })
        .then((res) => {
          const newGoals = [...goals];
  ...
  return (
    ...
    >
      {goalModal.screen === "category" && (
        <GoalTypeSelector handleCategory={handleCategory} />
      )}
      {goalModal.screen === "goal" && <GoalRegister handleGoal={handleGoal} />}
      {goalModal.screen === "habits" && (
        <HabitsRegister
